...
for i=1:length(idx)-1
    if strcmp(cell2mat(idx(i)),cell2mat(idx(i+1)))==1 || cell2mat(idx(i))==cell2mat(idx(i+1))
...
[idx_len1,cnt1] = count_result(idx1);
assert(sum(idx_len1)==length(idx1));
%for wrong answers
[idx_len2,cnt2] = count_result(idx2);
assert(sum(idx_len2)==length(idx2));

for i=4:4 %only use one object here to test
% for i=1:cnt1
    % find all coresponding wrong guesses for a correct guess
    % ti = [idx_wrong; worng_guess; idx_correct]
    ti = find_close_wrngus(i,idx_len1,idx_len2,cnt2,idx1,row1,guess1,idx2,row2,guess2);
...
function ti = find_close_wrngus(i,idx_len1,idx_len2,cnt2,idx1,row1,guess1,idx2,row2,guess2)
